...
// Mock org ID for development - replace with real auth
const MOCK_ORG_ID = "k17c0bsqr66p9y9qmfp88y2dq1752bvm" as Id<"organizations">;
...
export default function CalendarPage() {
  ...
  // Queries
  const deadlines = useQuery(api.calendar.listForCalendar, {
    orgId: MOCK_ORG_ID,
    includeCompleted: filters.showCompleted,
  });

  const categories = useQuery(api.calendar.getCategories, {
    orgId: MOCK_ORG_ID,
  });

  const assignees = useQuery(api.calendar.getAssignees, {
    orgId: MOCK_ORG_ID,
  });
  ...
  const handleDateSelect = useCallback((info: DateSelectArg) => {
    ...
    window.location.href = `/deadlines/new?date=${date}`;
  }, []);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <CalendarExportMenu orgId={MOCK_ORG_ID} />
          <Button
            variant="outline"
            size="sm"
            className="gap-1"
