...
export default function DashboardPage() {
  ...
  const dashboard = useQuery(
    ...
    orgId ? { orgId, viewMode, userId: undefined } : "skip"
  );
  ...
  // Loading state
  if (orgLoading || dashboard === undefined) {
    return (
  ...
  // No org selected - prompt to create/select one
  if (!orgId) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <p className="text-muted-foreground text-center max-w-md">
          Please create or select an organization using the switcher above to
          get started.
        </p>
      </div>
    );
